...
| Under Node the TUI paints nothing, sits idle, and Ctrl-C does not exit | **Config-dependent, not platform.** Reported on Apple Silicon 2026-08-26; the same Mac, same binary, same Node renders normally with a scratch `CLAUDE_CONFIG_DIR`, byte-identical write sequence to Linux. So macOS is not the variable - something in the real config is. A plain `--mcp-config` stdio server does **not** reproduce it here; the reported config additionally had plugin-provided MCP servers and a `SessionStart` hook spawned with a literal, unexpanded `${CLAUDE_PLUGIN_ROOT}` | Bisect: add `--strict-mcp-config --mcp-config '{"mcpServers":{}}'` to keep your real config and auth while suppressing MCP and plugins. To capture where it stops, see [Diagnosing a Node hang](#diagnosing-a-node-hang) |
