...
| **Runs under Node ≥ 24** | ✅ with `scripts/bun-shim.cjs`: `--version`, `--help`, `mcp list`, `config ls` byte-identical to Bun 1.3.14, `doctor` bar its `Path:` line ([findings.md](./findings.md) §11). ⚠️ command surface only — no agentic run under Node, anywhere | — never attempted (Node 24 exists on macOS; nobody has tried) | ⛔ |
