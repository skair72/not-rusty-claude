...
| **Runs under Node ≥ 24** | ✅ with `scripts/bun-shim.cjs`: `--version`, `--help`, `mcp list`, `config ls` byte-identical to Bun 1.3.14, `doctor` bar its `Path:` line ([findings.md](./findings.md) §11). ⚠️ no agentic run under Node anywhere; the interactive REPL does work (2026-08-26) | 🍎 the authenticated REPL renders on Apple Silicon, Node 24.19.0, reported 2026-08-26 — needed `wrapAnsi` and `YAML.parse` | ⛔ |
